...
class DetectionFastCollate:
    ...
    def __call__(self, batch):
        ...
        img_tensor = torch.zeros((batch_size, *batch[0][0].shape), dtype=torch.float32)
        for i in range(batch_size):
            img = batch[i][0]
            if isinstance(img, np.ndarray):
                img = torch.from_numpy(img)
            img = img.float() / 255.0
            # Save after scaling to [0, 1] (first 2 images only once)
            # if i < 2 and not hasattr(self, f'_scaling_debug_saved_{i}'):
            #     import torchvision.utils
            #     debug_scaling_path = f'debug_after_scaling_{i}.png'
            #     torchvision.utils.save_image(img, debug_scaling_path, normalize=False)
            #     setattr(self, f'_scaling_debug_saved_{i}', True)
            img_tensor[i] = img
            labeler_inputs = {}
        ...
        # Save debug images after collate (first 2 images, only once)
        # for j in range(min(batch_size, 2)):
        #     debug_collate_path = f'debug_after_collate_{j}.png'
        #     if not hasattr(self, f'_collate_debug_saved_{j}'):
        #         import torchvision.utils
        #         torchvision.utils.save_image(img_tensor[j], debug_collate_path, normalize=True)
        #         setattr(self, f'_collate_debug_saved_{j}', True)
        ...
        for next_input, next_target in self.loader:
            if self.device == "cuda":
                with torch.cuda.stream(stream):
                    ...
                    # Save debug images after prefetcher (first 2 images, only for first batch)
                    # if not hasattr(self, '_prefetcher_debug_saved_batch'):
                    #     import torchvision.utils
                    #     for j in range(min(next_input.shape[0], 2)):
                    #         debug_prefetcher_path = f'debug_after_prefetcher_{j}.png'
                    #         torchvision.utils.save_image(next_input[j], debug_prefetcher_path, normalize=False)
                    #     self._prefetcher_debug_saved_batch = True
                    if not first:
                        yield input, target
                    else:
                        first = False
