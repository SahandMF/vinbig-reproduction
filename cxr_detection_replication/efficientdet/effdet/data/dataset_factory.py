...
def create_dataset(name, root, splits=('train', 'val'), ann_file_override=None):
    ...
    if name.startswith('coco'):
        ...
        for s in splits:
            ...
            # Use override annotation file if provided
            if isinstance(ann_file_override, dict) and s in ann_file_override:
                ann_file = Path(ann_file_override[s])
            elif ann_file_override is not None and s == 'test':
                # Backward-compat: string override applies to test split only
                ann_file = Path(ann_file_override)
            else:
            ...
            if s == 'test':
                # For test split, use the 'test' subfolder directly
