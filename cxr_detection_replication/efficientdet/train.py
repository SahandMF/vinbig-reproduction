...
# Redirect stdout to a log file in output_dir
class TeeLogger:
...
parser.add_argument('--normalization', type=str, default='imagenet',
    choices=['imagenet', 'Vinbig', 'none', 'custom'],
    help='Normalization method to apply to input images.')
...
parser.add_argument('--ann-train-file', default=None, type=str,
                    help='Optional path to override COCO train annotations JSON')
parser.add_argument('--ann-val-file', default=None, type=str,
                    help='Optional path to override COCO val annotations JSON')
...
utils.add_bool_arg(parser, 'freeze-backbone', default=False, help='Freeze the backbone during training')
utils.add_bool_arg(parser, 'freeze-fpn', default=False, help='Freeze the FPN during training')
parser.add_argument('--save-samples', action='store_true', default=False,
                    help='Save example raw dataset images and a train batch image')
...
def main():
    ...
    args.normalization = 'none'   # <-- HARD CODE for now
    ...
    if args.freeze_fpn:
        for name, param in model.model.fpn.named_parameters():
            param.requires_grad = False
        logging.info("FPN frozen.")
    ...
    num_epochs = args.epochs  # ← force user-defined value
    start_epoch = 0
    ...
    loader_train, loader_eval, evaluator = create_datasets_and_loaders(args, model_config, normalization=args.normalization)
    ...
    if args.save_samples:
        ...
        torchvision.utils.save_image(
            images,
            os.path.join(args.output, 'check_train_batch.jpg'),
            padding=0,
            normalize=True,
        )
        print(f'Saved a visual sample batch to {os.path.join(args.output, "check_train_batch.jpg")}')
    
    # =====================================

    # Compute dataset class count robustly (handle 1-indexed labels)
    parser = loader_train.dataset.parser
    if getattr(parser, 'cat_id_to_label', None):
        dataset_num_classes = max(parser.cat_id_to_label.values())
        # If labels are 1-indexed, class count equals max label
    elif getattr(parser, 'cat_ids', None):
        # If cat_ids are used directly (may be 0- or 1-indexed); prefer length if contiguous
        try:
            dataset_num_classes = len(parser.cat_ids)
        except Exception:
            dataset_num_classes = parser.max_label
    else:
        dataset_num_classes = parser.max_label

    if model_config.num_classes < dataset_num_classes:
        logging.error(
            f'Model {model_config.num_classes} has fewer classes than dataset {dataset_num_classes}.')
        exit(1)
    if model_config.num_classes > dataset_num_classes:
        logging.warning(
            f'Model {model_config.num_classes} has more classes than dataset {dataset_num_classes}.')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if model_ema is not None:
                ...
                eval_metrics = validate(model_ema.module, loader_eval, args, evaluator, output_dir=output_dir, log_suffix=' (EMA)')
            else:
...
def create_datasets_and_loaders(
        ...
        collate_fn=None,
        normalization='imagenet',   # <-- ADD THIS
):
    ...
    # Support overriding train/val annotation files (for CV folds)
    ann_override = None
    if args.ann_train_file or args.ann_val_file:
        ann_override = {}
        if args.ann_train_file:
            ann_override['train'] = args.ann_train_file
        if args.ann_val_file:
            ann_override['val'] = args.ann_val_file

    dataset_train, dataset_eval = create_dataset(args.dataset, args.root, ann_file_override=ann_override)
    # ======= Save RAW Dataset Samples (Before Any Transformations) =======
    if args.save_samples:
        # Ensure output directory exists before saving sample images
        if getattr(args, 'output', None):
            os.makedirs(args.output, exist_ok=True)
        for i in range(2):
            img, target = dataset_train[i]
            img.save(os.path.join(args.output, f'raw_dataset_sample_{i}.png'))
...
def validate(model, loader, args, evaluator=None, output_dir='', log_suffix=''):
    batch_time_m = utils.AverageMeter()
    losses_m = utils.AverageMeter()
